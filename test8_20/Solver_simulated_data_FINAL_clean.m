...
param_HSI.reweight_abs_of_max = 1; 0.005; 1; % (reweight_abs_of_max * max) this is assumed true signal and hence will have weights equal to zero => it wont be penalised
...
%         pdfb_LRJS_Adapt_blocks_rwNL21_precond_new_sim(y, epsilons, A, At, aW, G, W, Psi_full, Psit_full, param_HSI2, X0);
    ...
    mkdir('results')
    save(['results/result_HyperSARA_','img=', num2str(img_size), '_tot=', num2str(tot), '_gamma=', num2str(gamma), '.mat'],'-v7.3','xsol', 'sol', 'X0', 'SNR', 'SNR_average', 'res','end_iter');
...
if flag_algo==2
    ...
    switch parallel_version
        ...
        case 'spmd4' % reference version, overlap between the facets underlying the nuclear norms
            ...
            [xsol,v0,v1,v2,weights0,weights1,proj,t_block,reweight_alpha,epsilon,t,rel_fval,nuclear,l21,norm_res,res,end_iter] = ...
                pdfb_LRJS_precond_NL21_sdwt2_spmd4_new(y_spmd, epsilon_spmd, ...
                A, At, aW_spmd, G_spmd, W_spmd, param_HSI, X0, Qx, Qy, Qc2, ...
                wlt_basis, L, nlevel, cell_c_chunks, ch(end), 0.5, bool_weights);
    ...
    mkdir('results/')
    save(['results/results_hyperSARA_', parallel_version, '_bool_weights=', num2str(bool_weights), '_img=', num2str(img_size), '_tot=', num2str(tot), '_Qx=', num2str(Qx), '_Qy=', num2str(Qy), '_Qc=', num2str(Qc2), '_gamma=', num2str(gamma),'.mat'],'-v7.3','xsol', 'sol', 'X0', 'SNR', 'SNR_average', 'res','end_iter');
