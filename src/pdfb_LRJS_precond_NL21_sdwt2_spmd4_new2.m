function [xsol,v0,v1,v2,weights0,weights1,proj,t_block,reweight_alpha,epsilon,t,rel_fval,nuclear,l21,norm_res,res,end_iter] = pdfb_LRJS_precond_NL21_sdwt2_spmd4_new2(y, epsilon, A, At, pU, G, W, param, X0, Qx, Qy, K, wavelet, L, nlevel, c_chunks, c, p, bool_weights)
...
rg_yo = domain_decomposition_overlap4(Qy, M, p);
rg_xo = domain_decomposition_overlap4(Qx, N, p);
...
% compute the number of redundant pixels for each dimension (x and y axis)
overlap_y = [(rg_yo(1,2)-rg_yo(1,1)+1); (rg_yo(2:end-1,2)-rg_yo(2:end-1,1)+1) - (rgy(:,2)-rgy(:,1)+1); (rg_yo(end,2)-rg_yo(end,1)+1); 0];
overlap_x = [(rg_xo(1,2)-rg_xo(1,1)+1); (rg_xo(2:end-1,2)-rg_xo(2:end-1,1)+1) - (rgx(:,2)-rgx(:,1)+1); (rg_xo(end,2)-rg_xo(end,1)+1); 0];
...
for q = 1:Q
    ...
    % define variable only for the valid facets... be careful with the
    % indices [to be written properly]
