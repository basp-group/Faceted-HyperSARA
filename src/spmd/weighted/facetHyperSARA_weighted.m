function [xsol,param,epsilon,t,rel_fval,nuclear,l21,norm_res_out,end_iter,SNR,SNR_average] = ...
    facetHyperSARA_weighted(y, epsilon, ...
...
%         fitswrite(xsol, './xsol.fits');
    ...
    if (param.use_reweight_steps && (rel_val(t) < param.reweight_rel_var) && ...
        ...
        spmd
            if labindex <= Qp.Value
                ...
                [weights1_, weights0_] = update_weights_overlap_weigthed(x_overlap, size(v1_), ...
                    ...
                    Ncoefs_q, dims_overlap_ref_q, offsetLq, offsetRq, reweight_alphap, w, sig, sig_bar);
        ...
        if (reweight_step_count == 0) || (reweight_step_count == 1) || (~mod(reweight_step_count,5))
            ...
            mkdir('./results/')
            m = matfile(['./results/facetHyperSARA_w_', name, '_', ...
