function [xsol,param,epsilon,t,rel_fval,nuclear,l21,norm_res_out,res,end_iter] = ...
    pdfb_LRJS_precond_NL21_sdwt2_spmd4_cst_overlap_weighted_dr_real(y, epsilon, A, At, pU, T, W, param, Qx, Qy, K, wavelet, L, nlevel, c_chunks, c, d, window_type)
...
spmd
    if labindex > Qp.Value
        res_ = compute_residual_images(xsol(:,:,c_chunks{labindex-Qp.Value}), yp, Tp, Ap, Atp, Wp);
    end
end
for k = 1 : K
    res(:,:,c_chunks{k}) = res_{Q+k};
