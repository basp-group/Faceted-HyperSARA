function [xsol,v0,v1,v2,weights0,weights1,proj,t_block,reweight_alpha,epsilon,t,rel_fval,nuclear,l21,norm_res,res,end_iter] = ...
    pdfb_LRJS_precond_NL21_sdwt2_spmd_serial_SARA2(y, epsilon, A, At, pU, G, W, param, X0, K, wavelet, nlevel, c_chunks, c)

% TO BE FINALIZED FOR COMPARISONS!
...
if isfield(param,'init_v0') || isfield(param,'init_v1')
    ...
    v1_{1} = param.init_v1{1};
    weights0_{1} = param.init_weights0{1};
    weights1_{1} = param.init_weights1{1};
...
for k = 1:K
    ...
    yp{1+k} = y{k};
    x_hat_i{1+k} = zeros(M, N, length(c_chunks{k}));
    Ap{1+k} = A;
    Atp{1+k} = At;
    Gp{1+k} = G{k};
    Wp{1+k} = W{k};
...
if isfield(param,'init_v2') % assume all the other related elements are also available in this case
    for k = 1:K
        v2_{1+k} = param.init_v2{k};
        proj_{1+k} = param.init_proj{k};
        t_block{1+k} = param.init_t_block{k};
    end
else
    for k = 1:K
...
for t = t_start : param.max_iter
    ...
    spmd
        if labindex == 1 % nuclear prior node
            ...
            tw = toc;
        else % data nodes, 2:K+1 (labindex > 1)
            tic
    ...
    if ~mod(t,100)
        ...
        % l21 norm
        l21 = sum(l21_norm_sara(xsol, Psit, s{1}));
