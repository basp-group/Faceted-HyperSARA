function [xsol,v0,v1,v2,g,weights0,weights1,proj,t_block,reweight_alpha,epsilon,t,rel_fval,nuclear,l21,norm_res,res,end_iter] = pdfb_LRJS_precond_NL21_sdwt2_spmd(y, epsilon, A, At, pU, G, W, Sp, Spt, param, X0, Qx, Qy, K, wavelet, L, nlevel, c_chunks, Psit)
...
for q = 1:Q
    for i = 1:K
        ...
        overlap{qi} = dims_overlap_ref(q, :); %max(dims_overlap{q}) - dims(q,:); % amount of overlap necessary for each facet        
    end
end
...
v0 = Composite(numworkers);
weights0 = Composite(Q);
v1 = Composite(numworkers);
weights1 = Composite(numworkers); % idem, can be created in parallel with spmd
for q = 1:Q
    p = prod(Ncoefs{q}, 2);
    if dirac_present
...
for t = t_start : param.max_iter
    ...
    spmd
        ...
        [v0, g0] = update_nuclear_spmd(v0, x_overlap(overlap_q(1)+1:end, overlap_q(2)+1:end, :), weights0, beta0.Value);
        
        [v1, g1] = update_l21_spmd(v1, x_overlap, weights1, beta1.Value, Iq, ...
        ...
        g1 = comm2d_reduce_wideband(g1, overlap_q, Qyp.Value, Qxp.Value, Kp.Value); % see how to update g1 from here...
    ...
    g = Spt(go) + sigma22*Ftx; % Spt could be applied on the workers directly (inter-worker communications)
    % Free memory
    %g0=[]; g1=[]; Ftx=[];
    ...
    if (param.use_reweight_steps && t == reweight_steps(rw_counts) && t < param.reweight_max_reweight_itr) || ...
        ...
        % communicate x_sol_split to the workers [sub-optimal]
        for qi = 1:numworkers
            [i, q] = ind2sub([K, Q], qi);
