function epsilon = update_epsilon(epsilon, t, t_block, rel_fval, norm_res, ...
    adapt_eps_tol_in, adapt_eps_steps, adapt_eps_rel_obj, adapt_eps_change_percentage, ...
...
for i = 1 : ci
    for  j = 1 : length(epsilon{i})
        ...
        if  norm_res{i}{j} > adapt_eps_tol_out * epsilon{i}{j}
            if t > t_block{i}{j} + param.adapt_eps_steps && rel_fval(t) < adapt_eps_rel_obj
