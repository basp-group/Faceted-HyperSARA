...
for iSpWin = 1:nchans
    dataSpWinloaded=load([data_file,num2str(spwins2image(iSpWin)),'.mat'],'y', 'u', 'v', 'w', 'nW', 'time', 'pos','pixelSize');
    ...
    u{iSpWin,1} = dataSpWinloaded.u; dataSpWinloaded.u =[];
    v{iSpWin,1} = dataSpWinloaded.v;dataSpWinloaded.v =[];
    w{iSpWin,1} = dataSpWinloaded.w;dataSpWinloaded.w =[];
    nW{iSpWin,1} = dataSpWinloaded.nW;dataSpWinloaded.nW =[];
    time{iSpWin,1}= dataSpWinloaded.time;dataSpWinloaded.time =[];
    pos{iSpWin,1}= dataSpWinloaded.pos;dataSpWinloaded.pos =[];
end
if ~isempty(l2bounds_filename)
    fprintf('\nLoading estimates of the l2-bounds ...')
    for iSpWin =1:nchans
        l2SpWinloaded = load(param_preproc.l2bounds_filename(spwins2image(iSpWin)),'sigmac','l2bounds');
        for iCh=1:numel(y{iSpWin})
            for iConfig = 1 : numel(pos{iSpWin}{iCh})
                try 
	            if ~flag_dr
                        l2bounds{iSpWin}{iCh}{iConfig} = l2SpWinloaded.l2bounds.cont{iConfig,iCh};
                    else,
			    l2bounds{iSpWin}{1}{1} =  l2SpWinloaded.l2bounds.gridded;
                    end
	...
	try
	   if ~flag_dr
		global_sigma_noise(iSpWin) = l2SpWinloaded.sigmac.cont;
   	   else
		 global_sigma_noise(iSpWin)=l2SpWinloaded.sigmac.gridded;
	   end
        catch,  global_sigma_noise(iSpWin) =1;
        end
...
for iSpWin =1:nchans
    % apply to data
    for  iCh =1:numel(y{iSpWin})
        y{iSpWin,1}{iCh}=(transpose(y{iSpWin,1}{iCh}));
        u{iSpWin,1}{iCh}=(transpose(u{iSpWin,1}{iCh}));
        v{iSpWin,1}{iCh}=(transpose(v{iSpWin,1}{iCh}));
        w{iSpWin,1}{iCh}=(transpose(w{iSpWin,1}{iCh}));
        nW{iSpWin,1}{iCh}=(transpose(nW{iSpWin,1}{iCh}));
        time{iSpWin,1}{iCh}=(transpose(time{iSpWin,1}{iCh}));
        pos{iSpWin,1}{iCh}=(transpose(pos{iSpWin,1}{iCh}));
        if ~flag_dr
            l2bounds{iSpWin,1}{iCh} = (transpose(l2bounds{iSpWin,1}{iCh}));
        end
    end
    y{iSpWin,1}=vertcat( y{iSpWin,1}{:});
    u{iSpWin,1}=vertcat( u{iSpWin,1}{:});
    v{iSpWin,1}=vertcat( v{iSpWin,1}{:});
    w{iSpWin,1}=vertcat( w{iSpWin,1}{:});
    nW{iSpWin,1}=vertcat( nW{iSpWin,1}{:});
    time{iSpWin,1}=vertcat( time{iSpWin,1}{:});
    pos{iSpWin,1}=vertcat( pos{iSpWin,1}{:});
    l2bounds{iSpWin,1} = vertcat(l2bounds{iSpWin}{:});
end
...
switch algo_version
    case 'sara'
        if flag_dr
            ...
            param_preproc.ch = spwins2image;
            [A, At, G, W, aW] = util_gen_wb_measurement_operator_dev_ad(u,v,w, nW, ...
                param_precond, param_blocking, 1, Nx, Ny, param_nufft,param_wproj,param_preproc);
            Sigma = [];
        end
    ...
    otherwise % 'hs' or 'fhs'
        ...
        % the algorithm used)
        yCmpst = Composite(); Sigma = Composite();
        if strcmp(algo_version, 'hs')
            spmd
                ...
                if flag_dr
                    % ! define Sigma (weight matrix involved in DR)
                    % ! define G as the holographic matrix
                    [A, At, G, W, aW,Sigma,yCmpst] = util_gen_dr_measurement_operator_dev_ad(yCmpst,u(local_fc), v(local_fc),w(local_fc),nW(local_fc), ...
...
end, clear param_blocking param_precond u v w nW;%% Free memory
...
if strcmp(algo_version, 'sara')
    if flag_computeOperatorNorm
        [Anorm, squared_operator_norm, rel_var, squared_operator_norm_precond, rel_var_precond] = util_operator_norm(G, W, A, At, aW, Ny, Nx, 1e-6, 200,flag_dr);%AD: changed tolerance to 1e-6 instead of 1e-8 Oo
