...
if flag_solveMinimization
    ...
    if strcmp(algo_version, 'sara')
        ...
        [xsol,param,v1,v2,g,weights1,proj,t_block,reweight_alpha,epsilon,t,rel_val,l11,norm_res,res,t_l11,t_master,end_iter] = ...
            ...
            fullfile(auxiliary_path,temp_results_name(nChannels)), x0, flag_homotopy, ncores_data, gam, flag_cirrus, regtype); %! in this case, ncores_data corresponds to the number of workers for the wavelet transform (9 maximum)
        ...
        switch algo_version 
            ...
            case 'cw'
                [xsol,param,epsilon,t,rel_val,nuclear,l21,norm_res_out,end_iter,snr_x,snr_x_average] = ...
                    ...
                    filter_length, nlevel, cell_c_chunks, channels(end), overlap_size, window_type, fullfile(auxiliary_path,warm_start(nChannels)), fullfile(auxiliary_path,temp_results_name(nChannels)), flag_homotopy, gam, gam_bar, sigma_noise_spmd, flag_cirrus, regtype);
            otherwise
