...
reweighting_alphap{1} = reweighting_alpha;
...
if init_flag
    for k = 1:K
        norm_res(1+k) = init_m.norm_res(k,1);
    end
...
if init_flag
    for k = 1:K
        v2_(1+k) = init_m.v2(k,1);
        proj_(1+k) = init_m.proj(k,1);
        t_block(1+k) = init_m.t_block(k,1);
    end
...
sigma22 = Composite();
for k = 1:K
    sigma22{1+k} = tau*sigma2(c_chunks{k});
end
...
for t = t_start : max_iter
    ...
    for i = 1:K
       xi{1+i} = xsol(:, :, c_chunks{i});
    end

    % update nuclear norm (on the master)
    ...
    spmd
        if labindex == 1 % nuclear prior node
            ...
            t_op = toc(tw);  
        % elseif labindex == 1 % l21 prior node (full SARA prior)
        %     tw = tic;
    ...
    if pdfb_converged 
        ...
        spmd
            ...
            %     weights1_ = update_weights_l21_serial(xsol, Psit_, weights1_, reweighting_alpha, sig_);
                ...
                res_ = compute_residual_images(xsol(:,:,c_chunks{labindex-1}), yp, Gp, Ap, Atp, Wp);
            end
        end

        for k = 1 : P
            weights1{k} = update_weights_l21_serial(xsol, Psit{k}, weights1{k}, reweighting_alpha, param.reweighting_sig);
        end
        ...
        if (reweight_step_count == 0) || (reweight_step_count == 1) || (~mod(reweight_step_count, param.backup_frequency))
            ...
            % data nodes
            for k = 1:K
                m.res(:,:,c_chunks{k}) = res_{1+k};
                res_{1+k} = [];
                m.v2(k,1) = v2_(1+k);
                m.proj(k,1) = proj_(1+k);
                m.t_block(k,1) = t_block(1+k);
                m.epsilon(k,1) = epsilonp(1+k);
                m.norm_res(k,1) = norm_res(1+k);
            end
            m.SNR = SNR;
