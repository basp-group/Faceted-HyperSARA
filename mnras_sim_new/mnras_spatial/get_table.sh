...
logpath=$(pwd)
outputfile=$(pwd)/output_parameters.txt
        ...
        for rwtype in dirty; do # dirty ground_truth
            for alph in 1e-1 1 10; do
                for alphbar in 1e-1 1 10; do
                    ...
                    filename=${logpath}/spatial_cygASband_Cube_512_1024_20_${algoversion}_${windowtype}_id=1_Qx=${Qx}_Qy=${Qy}_Qc=${Qc}_overlap=${overlap}_gam=${alph}_gambar=${alphbar}_rw=${rw}_homotopy=${homotopy}_rwt=${rwtype}_exptype=test_srf=2_snr=40_updatereg=${updatereg}_regtype=${regtype}.txt 
