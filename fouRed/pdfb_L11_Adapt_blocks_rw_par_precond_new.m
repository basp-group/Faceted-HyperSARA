function [xsol,v1,v2,g,weights1,t_block,reweight_alpha,epsilon,t,rel_fval,l11,norm_res,res] = pdfb_L11_Adapt_blocks_rw_par_precond_new(y, epsilon, A, At, G, W, Psi, Psit, param, ch, wterm, levelG, levelC)
...
for i = 1 : c
    g_f2 = zeros(No, 1);
    for q = 1 : length(Gt{i})
        g_f2(W{i}{q}) = g_f2(W{i}{q}) + Gt{i}{q} * tmp2{i}{q};
    end
    psf = real(At(g_f2));
    peak_psf{i} = max(psf(:));
    fprintf('Peak psf of channel %d: %e\n', i, peak_psf{i})
end    

% norm_conv_sara = zeros(param.max_iter, 1);
% xstar = param.xstar;
...
%         fitswrite(xsol, ['./results/xsol_sara_it', num2str(t), '.fits']);
%         save(['./results/conv_sara_it', num2str(t), '_gamma', num2str(param.gamma), '.mat'], '-v7.3', 'rel_fval', 'end_iter')
        ...
        if wterm
            prev_file = ['results/8basis/xsol_ch', num2str(ch), '_it', num2str(t-monitor_step), '_gamma', num2str(gamma), '_w', num2str(levelG), '_', num2str(levelC), '.fits'];
            if isfile(prev_file)
                delete(prev_file)
            end
            fitswrite(xsol, ['results/8basis/xsol_ch', num2str(ch), '_it', num2str(t), '_gamma', num2str(gamma), '_w', num2str(levelG), '_', num2str(levelC), '.fits']);
            
            prev_file = ['results/8basis/res_ch', num2str(ch), '_it', num2str(t-monitor_step), '_gamma', num2str(gamma), '_w', num2str(levelG), '_', num2str(levelC), '.fits'];
            if isfile(prev_file)
                delete(prev_file)
            end
            fitswrite(res, ['results/8basis/res_ch', num2str(ch), '_it', num2str(t), '_gamma', num2str(gamma), '_w', num2str(levelG), '_', num2str(levelC), '.fits']);
            
        else
            prev_file = ['results/8basis/xsol_ch', num2str(ch), '_it', num2str(t-monitor_step), '_gamma', num2str(gamma), '.fits'];
            if isfile(prev_file)
                delete(prev_file)
            end
            fitswrite(xsol, ['results/8basis/xsol_ch', num2str(ch), '_it', num2str(t), '_gamma', num2str(gamma), '.fits']);
            
            prev_file = ['results/8basis/res_ch', num2str(ch), '_it', num2str(t-monitor_step), '_gamma', num2str(gamma), '.fits'];
            if isfile(prev_file)
                delete(prev_file)
            end
            fitswrite(res, ['results/8basis/res_ch', num2str(ch), '_it', num2str(t), '_gamma', num2str(gamma), '.fits']);

        end
%         fitswrite(res, ['./results/res_sara_it', num2str(t), '.fits']);
...
%         weights1_old = weights1;
        ...
        % parallel for all bases, in fact only one channel in this case
        for i = 1 : c
            parfor k = 1:P
                d_val = abs(Psit{k}(xsol(:,:,i)));
                res_val = Psit{k}(res(:,:,i)/peak_psf{i});
                noise_val = std(res_val);
                weights1{k} = noise_val * max(reweight_alpha, 1) ./ (noise_val * max(reweight_alpha, 1) + d_val);
            end
        end
...
%         save(['conv_sara_it', num2str(t), '_reweight', num2str(reweight_step_count),...
%             '_gamma', num2str(param.gamma), '.mat'], '-v7.3', 'rel_fval', 'end_iter')
        
        if (reweight_step_count == 0) || (~mod(reweight_step_count,1))
            mkdir('./results')
            if wterm
                fitswrite(xsol, ['results/8basis/xsol_ch', num2str(ch),'_it', num2str(t), '_reweight', num2str(reweight_step_count), '_rw_alpha', num2str(reweight_alpha), '_gamma', num2str(gamma)...
                    '_w', num2str(levelG), '_', num2str(levelC), '.fits']);
                fitswrite(res, ['results/8basis/res_ch', num2str(ch),'_it', num2str(t), '_reweight', num2str(reweight_step_count), '_rw_alpha', num2str(reweight_alpha), '_gamma', num2str(gamma)...
                    '_w', num2str(levelG), '_', num2str(levelC), '.fits']);
            else
                fitswrite(xsol, ['results/8basis/xsol_ch', num2str(ch),'_it', num2str(t), '_reweight', num2str(reweight_step_count), '_rw_alpha', num2str(reweight_alpha), '_gamma', num2str(gamma)...
                    '.fits']);
...
%     norm_conv_sara(t) = norm(xsol(:) - xstar(:));
%     fprintf('convergence metric: %f\n', norm_conv_sara(t));
end
% save('norm_conv_sara.mat', '-v7.3', 'norm_conv_sara')
save('conv_sara_final.mat', '-v7.3', 'rel_fval', 'end_iter')
% Calculate residual images:
for i = 1 : c
    Fx = A(xsol(:,:,i));
    g2 = zeros(No,1);
