...
if compute_Anorm
    ...
    Ft = afclean( @(x) HS_fouRed_adjoint_operator(x, Bt));
...
%             [~,eps_b{i}{j}] = fb_nnls_blocks(yT{i}{j}, FIpsf{i}, FIpsf_t{i}, G{i}{j}, W{i}{j}, param_nnls);
%         end
%         if save_data
...
param_HSI.gamma0 = 1e-1;
param_HSI.gamma = 1e-3;  %convergence parameter L1 (soft th parameter)
...
if solve_HS
%     if usingReduction
%         [xsol,v0,v1,v2,g,weights0,weights1,t_block,reweight_alpha,epsilon,iterh,rel_fval,nuclear,l21,norm_res,res] = pdfb_LRJS_Adapt_blocks_rwNL21_par_sing_new_sim(ry_t, [Ny, Nx, length(ch)], epsilons_t, FIpsf, FIpsf_t, Sigma, Mask, Psi, Psit, param_HSI, X0);
        [xsol,v0,v1,v2,g,weights0,weights1,proj,t_block,reweight_alpha,epsilon,t,rel_fval,nuclear,l21,norm_res,res] = pdfb_LRJS_Adapt_blocks_rwNL21_par_sing_precond_new_sim(yT, [Ny, Nx, length(ch)], epsilon, A, At, pU, T, W, Psi, Psit, param,X0);
%         [xsol,v0,v1,v2,g,weights0,weights1,t_block,reweight_alpha,epsilon,iterh,rel_fval,nuclear,l21,norm_res,res] = pdfb_LRJS_Adapt_blocks_rwNL21_par_new_sim(ry_t, epsilons_t, F, Ft, T, W, Psi, Psit, param_HSI, X0);
%         save(['./simulated_data/data/result_snr' num2str(input_snr) '_FR' num2str(th) '.mat'],'-v7.3','xsol', 'v0', 'v1', 'v2', 'g', 'weights0', 'weights1', 'proj', 't_block','reweight_alpha', 'epsilon', 'iterh', 'rel_fval', 'nuclear', 'l21', 'norm_res', 'res');
...
%     else
%         [xsol,v0,v1,v2,g,weights0,weights1,t_block,reweight_alpha,epsilon,iterh,rel_fval,nuclear,l21,norm_res,res] = pdfb_LRJS_Adapt_blocks_rwNL21_par_new_sim(y_t, epsilons_t, A, At, G, W, Psi, Psit, param_HSI, X0);
%         save(['./simulated_data/data/result_snr' num2str(input_snr) '.mat'],'-v7.3','xsol', 'v0', 'v1', 'v2', 'g', 'weights0', 'weights1', 'proj', 't_block','reweight_alpha', 'epsilon', 'iterh', 'rel_fval', 'nuclear', 'l21', 'norm_res', 'res');
% 
%         c = size(xsol,3);
%         sol = reshape(xsol(:),numel(xsol(:))/c,c);
%         SNR = 20*log10(norm(X0(:))/norm(X0(:)-sol(:)))
