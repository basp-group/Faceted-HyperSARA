function [xsol,v0,v1,v2,g,weights0,weights1,t_block,reweight_alpha,epsilon,t,rel_fval,nuclear,l21,norm_res,res] = pdfb_LRJS_Adapt_blocks_rwNL21_par_sing_new_sim(y, imsize, epsilon, A, At, T, W, Psi, Psit, param,X0)

% This function solves:
%
% min || X ||_* + lambda * ||Psit(X)||_2,1   s.t.  || Y - T*A(X) ||_2 <= epsilon and x>=0
% where T*A is reduced measurement operator obtained from the Fourier reduction
% method
%
% Author: Ming Jiang
...
for t = t_start : param.max_iter
    ...
    s0 = diag(S0);
    v0 = v0 + xhatm - (U0*diag(max(s0 - beta0 * median(s0) * weights0, 0))*V0');
    % Free memory
    U0=[]; S0=[]; V0=[]; xhatm = [];
