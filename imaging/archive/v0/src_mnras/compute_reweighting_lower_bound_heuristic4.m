...
if strcmp(algo_version, 'hypersara')
    sig_bar = sqrt(Nx*Ny*sum(sigma_noise.^2./squared_operator_norm)/(Nx*Ny*nChannels));
else
    Q = Qx*Qy;
    ...
    for q = 1:Q
        ...
        Noq = prod(dims_o(q, :));
        sig_bar(q) = sqrt(sum(w(:).^2)*sum(sigma_noise.^2./squared_operator_norm)/(Noq*nChannels));
    end
end
