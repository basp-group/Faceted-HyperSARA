function [xsol,v0,v1,v2,g,weights0,weights1,proj,t_block,reweight_alpha,epsilon,t,rel_fval,nuclear,l21,norm_res,res] = pdfb_LRJS_Adapt_blocks_rwNL21_par_precond_new_sim_averaged(y, epsilon, A, At, pU, G, W, Av, Avt, Psi, Psit, param,X0)
...
if isfield(param,'init_v0')
    ...
    sol = reshape(xsol_ave(:),numel(xsol_ave(:))/c_ave,c_ave);
    v0 = zeros(size(sol));
...
if isfield(param,'init_v1')
    ...
    for k = 1:P
        ...
        v1{k} = zeros(size(Psit{k}(xsol_ave)));
...
sigma0 = 1.0/sqrt(param.nu0);
sigma1 = 1.0/sqrt(param.nu1);
sigma2 = 10/sqrt(param.nu2);
...
for t = t_start : param.max_iter
    ...
    xhatm = reshape(xhat_ave(:),numel(xhat_ave(:))/c_ave,c_ave);
    ...
    for k = 1:P
        f(k) = parfeval(@run_par_waverec, 3, v1{k}, Psit{k}, Psi{k}, xhat_ave, weights1{k}, beta1,c_ave);
    end
    ...
    for i = 1 : c
        ...
        g2 = zeros(No,1);
        for j = 1 : length(G{i})
            r2{i}{j} = param.ll * G{i}{j} * Fx(W{i}{j});
    ...
    g = sigma00*Avt(g0) + sigma11*Avt(g1) + sigma22*Ftx;
    % Free memory
    g0=[]; g1=[]; Ftx=[];
    ...
    if (param.use_reweight_steps && t == reweight_steps(rw_counts) && t < param.reweight_max_reweight_itr) || ...
        ...
        for k = 1 : P
            d_val1 = sqrt(sum(abs(Psit{k}(xsol_ave)).^2,2));
