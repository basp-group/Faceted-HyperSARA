...
if compute_Anorm_ch
    for i = ch
        %Anorm_ch(i) = pow_method_op(@(x) Gw{i}*A(x), @(x) At(Gw{i}' * x), [Ny Nx 1]);
...
param_HSI.gamma0 = 1;
param_HSI.gamma = 1e-2; 1e-3;   %convergence parameter L1 (soft th parameter)
...
for q = 1 : length(input_snr) * num_tests %number of tests x number of InSNRs
    ...
    if solve_HS
        ...
        [xsol,v0,v2,g,weights0,proj,t_block,reweight_alpha,epsilon,iterh,rel_fval,nuclear,norm_res,res] = ...
        pdfb_N_Adapt_blocks_rw_par_precond_new_sim(y_t{q}, epsilons_t{q}, A, At, aW, G, W, Psi, Psit, param_HSI, X0);
        if save_data
            save('./simulated_data/data/result_xmap_HS_rand_pos_na50_T200_seed1_new.mat','-v7.3','xsol', 'v0', 'v1', 'v2', 'g', 'weights0', 'weights1', 'proj', 't_block','reweight_alpha', 'epsilon', 'iterh', 'rel_fval', 'nuclear', 'l21', 'norm_res', 'res');
        end
    ...
    if solve_1B
        for i = ch
            ...
            [xsol,v1,v2,g,weights1,proj,t_block,reweight_alpha,epsilon,iterh,rel_fval,l11,norm_res,res] = pdfb_L11_Adapt_blocks_rw_par_precond_new_sim({y_t{q}{i}},{epsilons_t{q}{i}}, A, At, {aW{i}}, {G{i}}, {W{i}}, Psi, Psit, param_HSI, X0(:,i));
            if save_data
...
fitswrite(xsol,'./xsol_10000.fits')
