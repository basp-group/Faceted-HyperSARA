...
with open(parameter_file_full_path, "r") as csvfile:
    ...
    for job in reader:
        ...
        os.makedirs(
            "results/{0}/logs".format(job[0]), exist_ok=True
        )  # if the directory already exists, do nothing
        ncores = np.minimum(
            int(job[2]) * int(job[3]) + int(job[10]) + 1, 36
        )  # max number of cpus = 36
        # ncores = max(Qx*Qy + n_data_cores + 1, 36) if solver limited to a single node

        slurm_command = """sbatch --job-name={0} --ntasks-per-node={24} \
            ...
            -v --export=ALL,imgname={0},Qx={2},Qy={3},Qc={4},nchannels={1},p={6},snr={7},algoversion={8},wintype={9},ncdata={10},ind={11},overlapsize={12},gencube={13},gencov={14},genvis={15},genundersampledcube={16},computenorm={17},solve={18},cubepath={19},covpath={20},nreweights={21},gamma={22},rw={23} \
