function [xsol,v0,v2,g,weights0,proj,t_block,reweight_alpha,epsilon,t,rel_fval,nuclear,norm_res,res] = pdfb_N_Adapt_blocks_rw_par_precond_new_sim_split(y, epsilon, A, At, pU, G, W, Sp, Spt, Psi, Psit, param, X0)
...
%Step size primal
tau = 0.99/(sigma0*param.nu0 + sigma2*param.nu2);
...
for t = t_start : param.max_iter
    ...
    if ~mod(t,25)
        ...
        if (param.verbose >= 1)
            fprintf('Iter %i\n',t);
            fprintf('N-norm = %e, rel_fval = %e\n', nuclear(t), rel_fval(t));
