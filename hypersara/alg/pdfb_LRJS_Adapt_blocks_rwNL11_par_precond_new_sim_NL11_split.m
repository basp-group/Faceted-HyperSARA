function [xsol,v0,v1,v2,g,weights0,weights1,proj,t_block,reweight_alpha,epsilon,t,rel_fval,nuclear,l11,norm_res,res] = pdfb_LRJS_Adapt_blocks_rwNL11_par_precond_new_sim_NL11_split(y, epsilon, A, At, pU, G, W, Sp, Spt, Spt2, Psi, Psit, param, X0)
...
if isfield(param,'init_weights1')
    ...
    for k = 1:P
        for i = 1 : length(x_split)
            weights1{k}{i} = ones(size(v1{k}{i},1),c_chunk(i));
        end
    end
...
for t = t_start : param.max_iter
    ...
    g = sigma00*Spt(g0) + sigma11*Spt2(g1) + sigma22*Ftx;
    % Free memory
    g0=[]; g1=[]; Ftx=[];
    ...
    if ~mod(t,25)
        ...
        if (param.verbose >= 1)
            fprintf('Iter %i\n',t);
            fprintf('N-norm = %e, L11-norm = %e, rel_fval = %e\n', nuclear(t), l11(t), rel_fval(t));
    ...
    if (param.use_reweight_steps && t == reweight_steps(rw_counts) && t < param.reweight_max_reweight_itr) || ...
        ...
        for k = 1 : P
            for i = 1 : length(xhat_split)
            d_val1 = abs(Psit{k}(xsol_split{i}));
...
function [v1_, u1_, l11_] = run_par_waverec(v1_, Psit, Psi, xhat_split, weights1_, beta1)

l11_ = 0;
for i = 1 : length(xhat_split)
    r1 = v1_{i} +  Psit(xhat_split{i});
    v1_{i} = r1 - sign(r1).*max(abs(r1) - beta1*weights1_{i},0);
    u1_{i} = Psi(v1_{i});
