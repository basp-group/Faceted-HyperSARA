...
%% Averaging operator
ave = 3;
cent = [round(ave/2):ave:length(ch)];
step = floor(ave/2);

Av = @(x) Average_forward_operator(x,cent,step,Ny,Nx);
Avt = @(x) Average_adjoint_operator(x,cent,step,ave,Ny,Nx,length(ch));

Av_norm = pow_method_op(Avt,Av,[Ny Nx length(ch)]);
...
% 
% Av_psit = @(x) Average_sparsity_forward_operator(x,Av,Psit);
% Avt_psi = @(x) Average_sparsity_adjoint_operator(x,Avt,Psi);
% 
% Pnorm = pow_method_op(Av_psit, Avt_psi,[Ny Nx length(ch)]);
...
%% L21 + Nuclear
if solve_HS
    [xsol,v0,v1,v2,g,weights0,weights1,proj,t_block,reweight_alpha,epsilon,iterh,rel_fval,nuclear,l21,norm_res,res] = pdfb_LRJS_Adapt_blocks_rwNL21_par_precond_new_sim_averaged(y_t{q}, epsilons_t{q}, A, At, aW, G, W, Av, Avt, Psi, Psit, param_HSI, X0);
    if save_data
...
%%
save('result_66ch_ave3_noHI_nu2x10.mat','-v7.3','xsol', 'sol', 'X0', 'SNR', 'SNR_average', 'res');

fitswrite(xsol,'xsol_ave3_noHI_nu2x10.fits')
