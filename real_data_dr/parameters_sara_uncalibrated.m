...
if compute_Anorm
    ...
    save('Anorm_ch.mat','-v7.3', 'Anorm_ch');
else
    load('Anorm_ch.mat'); 
end
...
param_HSI.reweight_abs_of_max = 0.1; 0.005; % (reweight_abs_of_max * max) this is assumed true signal and hence will have weights equal to zero => it wont be penalised
...
if flag_algo == 0
    util_create_pool(34);
    maxNumCompThreads(34);
    for i = 1
        i
        [xsol(:,:,i),v1,v2,g,weights1,proj,t_block,reweight_alpha,epsilon,iterh,rel_fval,l11,norm_res,res(:,:,i),end_iter{i}] = ...
        pdfb_L11_Adapt_blocks_rw_precond_new_sim({yb{i}},{eps_b{i}}, A, At, {aW{i}}, {G{i}}, {W{i}}, Psi_full, Psit_full, param_HSI, Anorm_ch(i),i);
    ...
    save(['./results/result_SARA_' num2str(ch_num) '_' num2str(ind) '_' num2str(gamma) '.mat'],'-v7.3','xsol','res','end_iter');
