function func_nnls_dr_real_data_newData_wterm(inputdir, outputdir, nb_aggch, chInd, subProb, reduction_version, realdatablocks, enable_klargestpercent, fouRed_gamma, fouRed_type, levelG, levelC)
...
%                 ind2 = nbRows;
%             end
%             slicesIdx(ind1:ind2, 1) = m;  
%         end
%         Ggroups = splitapply(@(x){x}, G, slicesIdx);
%         startH = tic;
%         parfor m = 1:numworkers
%             tmp{m} = Ggroups{m}'* Ggroups{m};
%         end
%         fprintf('H computation time: %e\n',toc(startH))
%         clear Ggroups
    startH = tic;
%     Hl = sparse(ox*Nx*oy*Ny,ox*Nx*oy*Ny);
%     for m = 1:numworkers
%         ind1 = (m-1)*slices + 1;
%         ind2 = m * slices;
%         if ind2 > nbRows
