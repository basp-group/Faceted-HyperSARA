...
param_HSI.total_reweights = 10; -1; % -1 if you don't want reweighting
...
param_HSI.reweight_min_steps_rel_obj = 500; % min num of iter between reweights
...
param_HSI.elipse_proj_min_iter = 1; % mi
n num of iter for the FB algo that implements the preconditioned projection onto the l2 ball
...
%         pdfb_LRJS_Adapt_blocks_rwNL21_par_precond_new_sim(y, epsilons, A, At, aW, G, W, Psi, Psit, param_HSI2, X0);
    ...
    save(['results/results_hyperSARA_', 'img=', num2str(img_size), '_tot=', num2str(tot), '_Qc=', num2str(Qc), '_ind=', num2str(ind), '_Qc2=', num2str(Qc2), '_gamma=', num2str(gamma), '.mat'],'-v7.3','xsol', 'sol', 'X0', 'SNR', 'SNR_average', 'res','end_iter');
...
if flag_algo == 0
    ...
    save(['results/results_SARA_', 'img=', num2str(img_size), '_tot=', num2str(tot), '_Qc=', num2str(Qc), '_ind=', num2str(ind), '_Qc2=', num2str(Qc2), '_gamma=', num2str(gamma), '.mat'],'-v7.3','xsol', 'sol', 'X0', 'SNR', 'SNR_average', 'res','end_iter','Time_iter_average');
...
%                 wlt_basis, L, nlevel, cell_c_chunks, ch(end));
    ...
    mkdir('results/')
    %save(['results/results_hyperSARA_', parallel_version, '_img=', num2str(img_size), '_tot=', num2str(tot), '_Qx=', num2str(Qx), '_Qy=', num2str(Qy), '_Qc=', num2str(Qc2), '.mat'],'-v7.3','end_iter');

    save(['results/results_hyperSARA_', parallel_version, '_img=', num2str(img_size), '_tot=', num2str(tot), '_Qc=', num2str(Qc), '_ind=', num2str(ind), '_Qx=', num2str(Qx), '_Qy=', num2str(Qy), '_Qc2=', num2str(Qc2), '_gamma=', num2str(gamma), '.mat'],'-v7.3','xsol', 'sol', 'X0', 'SNR', 'SNR_average', 'res', 'end_iter');
